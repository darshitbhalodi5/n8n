  ...
  const loadConnections = React.useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/integrations/slack/connections`,
        {
          headers: {
  ...
  const handleTestWebhook = async (
    ...
    try {
      ...
      const testResponse = await fetch(
        `${API_BASE_URL}/integrations/slack/test`,
        {
          method: "POST",
          headers: {
      ...
      if (!testResponse.ok) {
        ...
        const errorMessage =
          errorData.error?.message || "Failed to test webhook";
  ...
  const handleSaveWebhook = async () => {
    ...
    try {
      ...
      const saveResponse = await fetch(
        `${API_BASE_URL}/integrations/slack/webhooks`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteConnectionConfirm = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(
          errorData.error?.message || "Failed to delete connection"
        );
      }
            ...
            <div className="space-y-3 p-3 border border-border rounded-lg bg-secondary/20">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  disabled={
                    isProcessing || !webhookUrl.trim() || !testMessage.trim()
                  }
                  variant="outline"
                ...
                <Button
                  ...
                  disabled={
                    isProcessing || !webhookUrl.trim() || !isTestSuccessful
                  }
                  className="flex-1 gap-2"
                  size="sm"
                  title={
                    !isTestSuccessful ? "Please test the webhook first" : ""
                  }
                >
              ...
              {!isTestSuccessful && !isProcessing && (
                <Typography variant="caption" className="text-muted-foreground">
                  💡 Click &quot;Test&quot; to verify your webhook works before
                  saving
                </Typography>
              )}
            </div>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this webhook connection? This
