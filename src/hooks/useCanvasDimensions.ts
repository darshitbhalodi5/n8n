 ...
 * Provides reactive dimensions based on window size and layout configuration
 */
export function useCanvasDimensions(): CanvasDimensions {
    ...
    return useMemo(() => {
        ...
        const isTablet = width >= LAYOUT_CONSTANTS.BREAKPOINTS.MOBILE && width < LAYOUT_CONSTANTS.BREAKPOINTS.DESKTOP;
        const isDesktop = width >= LAYOUT_CONSTANTS.BREAKPOINTS.DESKTOP;

        // Calculate sidebar widths based on breakpoints
        const categoryWidth = width >= LAYOUT_CONSTANTS.BREAKPOINTS.DESKTOP
            ? LAYOUT_CONSTANTS.SIDEBAR_WIDTHS.CATEGORY.DESKTOP
            : LAYOUT_CONSTANTS.SIDEBAR_WIDTHS.CATEGORY.TABLET;

        const blocksWidth = width >= LAYOUT_CONSTANTS.BREAKPOINTS.DESKTOP
        ...
        const configWidth = width >= LAYOUT_CONSTANTS.BREAKPOINTS.DESKTOP
