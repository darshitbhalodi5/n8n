 ...
 * Centralized configuration for Safe wallet onboarding
 * This module handles mode detection, backend handshake, and validation
 */

import { API_CONFIG } from "@/config/api";
import type { ChainDefinition } from "@/web3/chains";
import { USE_TESTNET_ONLY as FRONTEND_USE_TESTNET_ONLY, getSelectableChains } from "@/web3/chains";
...
export async function fetchBackendRuntimeConfig(): Promise<BackendRuntimeConfig> {
  const response = await fetch(`${API_CONFIG.BASE_URL}/meta/runtime-config`);
 ...
 */
export function assertModeMatchOrThrow(backendConfig: BackendRuntimeConfig): void {
