 ...
 *
 * We keep explicit static imports for all known blocks (good for tree-shaking),
 * and layer an optional auto-discovery helper on top using `require.context`
 * when available (Webpack-compatible environments). If dynamic discovery is
 * not available, everything still works via the static list.
 */
